...
////////////////////////////////////////////////////////////
// $Id: configur.cc,v 2.6 1998/04/01 21:05:42 jelasity Exp $
// configur.cc
// contains the definition of class Configure
////////////////////////////////////////////////////////////
// modification history:
//	jelasity 98 01 10: puffer size changed in ReadParFile
//	jelasity 98 01 17: command line interface added
//	jelasity 98 01 23: levels == 1 case handled
//	jelasity 98 01 25: last r approximation finer
////////////////////////////////////////////////////////////
...
double Configure::GetValue(char *prompt, char *helpmsg, double def) {
  ...
  while (!ValueRead) {
    ...
    case 'q': // cancel config
      message("Configuration cancelled.", MSG_INFORMATION);
    ...
    default: // try to read value
      ValueRead = sscanf(puffer, "%lf", &result) == 1;
    };
  };
...
FILE *Configure::GetOpenFile(char *prompt, char *helpmsg) {
  ...
  while (!ValueRead) {
    ...
    default:                          // try to read value
      ...
      ValueRead = (result = fopen(puffer, "rt")) != NULL;
    };
  };
...
void Configure::ReadParFile(FILE *parfile) {
  ...
  char puffer[10002], ValueRead, *pos, white[15] = " \n\v\b\r\f\t";
  long i;
  double d;
...
void Configure::GetParameterVector() {
  ...
  do
          newparnum = (long) GetValue(prompt, PARNUM_HE, (double)
  ini->paramnum); while (!Fail() && newparnum < 0); if (Fail()) return; if
  (newparnum == 0) { ini->paramnum = 0; if (ini->param != NULL) delete
  ini->param; ini->param = NULL; return;
  };
  ...
  if (ini->paramnum != newparnum || ini->param == NULL) {
          ...
          for (i = 0; i < newparnum; ++i)
                  ini->param[i] = 0.0;
  };

  // --- reading values ----------------------------------
  for (i = 0; !Fail() && i < newparnum; ++i) {
          sprintf(prompt, "%ld. constant parameter (%lf): ", i + 1,
                          ini->param[i]);
...
void Configure::GetBoundVectors(long newdim) {
  ...
  if (ini->dimension != newdim || ini->upb == NULL || ini->lowb == NULL) {
    ...
    for (i = 0; i < newdim; ++i)
      ini->upb[i] = ini->lowb[i] = 0.0;
  };

  // --- reading values ----------------------------------
  for (i = 0; !Fail() && i < newdim; ++i) {
    sprintf(prompt, "%ld. component of lower bound (%lf): ", i + 1,
            ini->lowb[i]);
    ini->lowb[i] = GetValue(prompt, "", ini->lowb[i]);
  };
  for (i = 0; !Fail() && i < newdim; ++i) {
    sprintf(prompt, "%ld. component of upper bound (%lf): ", i + 1,
            ini->upb[i]);
    do
      d = GetValue(prompt, "", ini->upb[i]);
...
void Configure::SetMissingNu() {
  ...
  evals1 = ini->maxevals;
  for (i = 1; i < ini->levels; ++i)
    evals1 -= ini->newspecevals[i - 1];
  if (evals1 < 0) {
    message("All evaluations (N) too small, parameters not set.", MSG_ERROR);
    FailFlag = 1 == 1;
    return;
  };
...
void Configure::SetVectors(long miss) {
  ...
  if (miss != MISSING_M && miss != MISSING_NU && miss != MISSING_LAST_R)
    for (i = 1; i < ini->levels; ++i)
      ini->evals[i] =
          (unsigned long)(Master::M(i, ini) * ini->r[0] * ini->threshold /
...
void Configure::ReAllocVectors(long levels) {
  ...
  if (ini->evals != NULL)
    delete ini->evals;
  if (ini->newspecevals != NULL)
    delete ini->newspecevals;
  if (ini->r != NULL)
    delete ini->r;
...
WARN:
  ...
  for (long i = 1; i < ini->levels; ++i)
    if (ini->Evals(i) / (long)Master::M(i, ini) == 0) {
      sprintf(prompt, "0 evaluations for level %ld species!", i + 1);
      message(prompt, MSG_INFORMATION);
    };
};
...
void Configure::DisplayConfig() {
  ...
  do
    l = (long)GetValue(prompt, TYPE_HE, (double)ini->type);
  ...
  do
    l = (long)GetValue(prompt, DIM_HE, (double)ini->dimension);
  while (!Fail() && l < 1);
  ...
  ini->maxevals =
      (unsigned long)GetValue(prompt, MAXEV_HE, (double)ini->maxevals);

  sprintf(prompt, "%s (%ld): ", MAXSPEC_T, ini->maxspecnum);
  do
    l = (long)GetValue(prompt, MAXSPEC_HE, (double)ini->maxspecnum);
  while (!Fail() && l < 1);
  ini->maxspecnum = l;

  sprintf(prompt, "%s (%lg): ", THR_T, ini->threshold);
  do
    d = GetValue(prompt, THR_HE, ini->threshold);
  while (!Fail() && d <= 0.0);
  ini->threshold = d;

  sprintf(prompt, "%s (%ld): ", LEVELS_T, (long)ini->levels);
  do
    l = (long)GetValue(prompt, LEVELS_HE, (double)ini->levels);
  while (!Fail() && l < 1);
  ini->levels = (short)l;
  ...
  do
    d = GetValue(prompt, LAST_R_HE, last_r);
  while (!Fail() && (d <= 0.0 || d >= ini->prototype->Diameter(ini)));
  last_r = d;

  sprintf(prompt, "Parameter to ignore (2): ");
  do
    l = (long)GetValue(prompt, MISS_HE, 2.0);
  while (!Fail() && !VALID_MISS(l));

  if (!Fail())
    AutoParameters(l); // sets the remaining vectors
};
...
void Configure::ParamConfig(int argc, char **argv) {
  ...
  if (!FailFlag)
    ...
    message("Command line configuration failed.", MSG_ERROR);
};
...
void Configure::ConfigUpdating(char *ininame, int argc, char **argv) {
