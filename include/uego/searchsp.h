...
protected:
  ...
  virtual double PreciseValue() = 0; // No optimization in objective function
  // --- error handling flag ----------------------------------------
  char FailFlag;
...
public:
  ...
  virtual double PreciseUpdateValue() {
    return (value = Value());
  }; // No optimization in objective function
  ...
  virtual void Save(FILE *, bool) = 0;

  char Fail() { return FailFlag; };
  virtual void GetX(double *y) = 0;

  // ----- y is denormalized values. Inside normalization process must do it
  virtual void SetX(double *y) = 0;
};
